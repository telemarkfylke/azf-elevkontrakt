...
const postFormInfo = async (formInfo, isMock) => {
  ...
  if (isMock !== true && document.isError !== 'true' && document.isNonFixAbleError !== 'true') {
    ...
    if (fnr && kontraktType) {
      ...
      // Siste sikkerhetssjekk før dokumentet kan postes til kontrakter: fakturaInfo skal alltid
      // matche kontraktType. Poster til error-collection for manuell gjennomgang istedenfor å
...
const moveAndDeleteDocument = async (documentId, targetCollection, sourceCollection, deps = {}) => {
  ...
  const determineSourceCollectionName = (sourceCollection) => {
    ...
    } else if (sourceCollection === 'error') {
      return mongoDB.errorCollection
    } else {
      return null
    }
  }
  ...
  const moveDocumentToTargetCollection = async (documentId, targetCollection, sourceCollection) => {
    ...
    } else if (targetCollection === 'regular') {
      collection = mongoDB.contractsCollection
    }
